...
pathIn = r"E:\pycharm_project\TCP 10 25\chazhi_result.txt"
pathOut = r"E:\pycharm_project\MapData\change_result_ouput.txt"
pathout2 = r"E:\pycharm_project\TCP 10 25\lat_lng.txt"  # 调试直接输出，不转换成html文件所需格式
result = []
count = 0
if __name__ == '__main__':
    ...
    try:
        for i in range(len(lng)):
            ...
            # 取得小车上传的经纬度数据，不转成html文件所需格式，仅供测试
            # gcj02_to_bd09
            # pathout2
            converted = wgs84_to_bd09(lng[i], lat[i])
            with open(pathout2, 'a') as f:
                print(str(converted))
                # f.writelines(str(converted) + "\n")
                f.writelines(str(converted[0])+"\n")  ####
                f.writelines(str(converted[1]) + "\n")
                ###########----------------改成html文件所需的格式
                count += 1
